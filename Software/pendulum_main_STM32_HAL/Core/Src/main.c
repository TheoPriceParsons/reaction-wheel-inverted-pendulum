...
/* USER CODE BEGIN PD */
#define DT   0.002f
#define V_SUPPLY 3.0f   // change to 12.0f V
//physics sims constants
#define G       9.81f
#define M_PEND  0.5f
#define L       0.3f
#define R_M     2.4f
...
void setMotor(float Vcmd);
static inline float wrap(float angle);
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
//control loop constant
volatile uint8_t doControl = 0;
//gains from sim
...
int main(void)
{
  ...
  while (1)
  {
	  if (doControl) {
		  ...
		  //we measure theta
		  int16_t count = (int16_t)TIM4->CNT;
		  float theta = ((float)count/(2048.0f * 4.0f))*2.0f*M_PI;
		  float delta_meas = wrap(theta - M_PI);
		  //we measure Omega
		  int32_t raw = (int32_t)TIM2->CNT;
		  float omega = ((float)raw / 48.0f) * (2.0f * M_PI) / DT;
		  TIM2->CNT = 0;
		  //sim + control
		  float x_hat_new[2];
		  ...
		  float E = 0.5f * M_PEND * L * L * x_hat[1] * x_hat[1] - M_PEND * G * L * cosf(theta);
		  //our goal
		  float E_star = M_PEND * L * G;
		  //Bang Bang voltage:
		  float V_su;
		  if (fabsf(x_hat[1])<1e-6f) {
		        V_su = 0.3f*V_SUPPLY;
		   }
		   else if (E<E_star) {
		        V_su = -V_SUPPLY*x_hat[1]/(fabsf(x_hat[1]));
		   }
		   else {
		        V_su = 0;
		   }
		   ...
		   if (++uart_div >= 10) {
		       ...
		       int len = snprintf(buf, sizeof(buf), "%.3f,%.3f,%.3f,%.3f\r\n",
		                          delta_meas, x_hat[1], omega, V_cmd);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2|GPIO_PIN_10|GPIO_PIN_4, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : PB2 PB10 PB4 */
...
void setMotor(float Vcmd){
	Vcmd=fmaxf(-V_SUPPLY,fminf(V_SUPPLY,Vcmd));
	uint32_t duty = (uint32_t)(fabsf(Vcmd)/V_SUPPLY*8999.0f);
	if (Vcmd>0.0f) {
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, GPIO_PIN_SET);//INA
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2,  GPIO_PIN_RESET);//INB
	}
	else {
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_10, GPIO_PIN_RESET);//INA
		HAL_GPIO_WritePin(GPIOB, GPIO_PIN_2,  GPIO_PIN_SET);//INB
	}
